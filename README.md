...
- **表現層細節**：真實 strip 滾動、物件池、逐欄停輪與回彈、anticipation、逐線中獎演出、免費遊戲倍數。
...
| 其他 | Autoplay（10 / 25 / 50 次）、Turbo 模式、快速停止、略過演出 |
...
目標：**RTP 96.0%（允收 95.5% ~ 96.5%）、hit frequency 25% ~ 32%、中高波動**。

### 1. 為什麼用 reel strip，而不是逐格權重抽樣

每一欄是一條固定的符號序列（50 ~ 53 格），每次 spin 為每欄抽一個停輪位置，取連續 3 格。

- 這是實體與線上 slot 的實際做法，數學與畫面是**同一個模型**：滾輪轉動時顯示的就是真實 strip，停輪位置直接對應結果，不需要「轉完再換圖」。
...
1. 以一組**刻意偏高 RTP** 的組成為起點（高賠付與 WILD 較集中）。這份賠付表的低賠付倍數偏低（3 連只有 2 倍線注），若不提高集中度，RTP 只有三成左右。
...
### 4. 波動性：用堆疊（stack）調整 hit frequency

RTP 只取決於組成，**排列方式不會改變 RTP，但會改變中獎的分佈**。把同一符號排成 4 格一組的堆疊，中獎會集中在同一次 spin 的多條線上。以最終組成、不同堆疊長度各模擬 100 萬次：

| 同一組成的排列方式 | 理論 RTP | hit frequency | 單次 spin 標準差 |
|---|---|---|---|
| 無堆疊 | 95.98% | 61.45% | 3.78 |
| 2 格堆疊 | 95.98% | 41.11% | 4.47 |
| 3 格堆疊 | 95.98% | 31.66% | 5.06 |
| **4 格堆疊（採用）** | **95.98%** | **28.57%** | **5.74** |

同樣的回報率，從「經常小贏」變成「較少中獎、但一中就是多條線」，這就是中高波動的手感。
...
|---|---|---|
| **總 RTP** | **96.43%** | **95.98%** |
| 一般遊戲 RTP | 76.31% | 76.29% |
| 免費遊戲 RTP | 20.11% | 19.69% |
| Hit frequency | 28.57% | — |
| 免費遊戲觸發頻率 | 每 170.7 轉 | 每 170.6 轉 |
| 每次觸發平均免費旋轉 | 10.83 次 | 10.80 次 |
| 最大單次贏分（含觸發的免費遊戲） | 1,382x | — |
| 標準差（單次 spin） | 5.74 | — |
| RTP 95% 信賴區間 | ± 1.12% | — |
...
|---|---|---|---|---|---|
| 71.43% | 12.66% | 11.96% | 3.19% | 0.56% | 0.20% |

> 在標準差 5.7 的波動下，100 萬次模擬的 RTP 95% 信賴區間約 ±1.1%，單一 seed 的模擬結果本來就會在理論值附近浮動。
> 這也是另外做精確解析的原因：**調校用理論值，模擬用來驗證實作與理論一致**（一般遊戲 RTP、觸發頻率、平均免費旋轉次數的模擬值都與理論值吻合到小數點後一位）。
...
- **物件池**：每欄固定 5 個符號節點（3 可視 + 上下各 1 緩衝）。滾出下界的節點回收到最上方，並換成 strip 上的下一個符號。整局遊戲不會再 `instantiate` 或銷毀任何符號節點，連續旋轉不會產生 GC 壓力或節點累積。
- **停輪對齊**：因為滾動時顯示的就是真實 strip，停輪只要把 `pos` 對齊到引擎給的 `stopIndices`，結果符號自然會依序從上方進入可視區。畫面與結果在結構上就不可能不一致，而不是靠「停下來那一刻換貼圖」。
...
`tools/gen-symbols.ts` 以程式組出 SVG，再用 resvg 光柵化為 256x256 PNG。所有符號共用相同的外框、圓角與內縮留白，讓盤面上的視覺重量一致。背景為 1920x720 的純裝飾圖，用來填補寬螢幕左右多露出的區域。

### 解析度

設計解析度 1280x720，`fitHeight`（固定高度）。所有遊戲內容都位於中央 1280x720 的安全區內，UI 以中央為基準排列；比 16:9 寬的螢幕只會多露出背景，比 16:9 窄的螢幕則裁切左右兩側的背景。
